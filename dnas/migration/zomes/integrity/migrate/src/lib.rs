...
pub fn validate(op: Op) -> ExternResult<ValidateCallbackResult> {
    match op.flattened::<EntryTypes, LinkTypes>()? {
        ...
        FlatOp::RegisterCreateLink {
            ...
            match link_type {
                LinkTypes::AllHashChanges => {
                    validate_create_link_all_hash_changes(
                        action,
                        base_address,
                        target_address,
                        tag,
        ...
        FlatOp::RegisterDeleteLink {
            ...
            match link_type {
                LinkTypes::AllHashChanges => {
                    validate_delete_link_all_hash_changes(
                        action,
                        original_action,
                        base_address,
                        target_address,
        ...
        FlatOp::StoreRecord(store_record) => {
            match store_record {
                ...
                OpRecord::UpdateEntry {
                    ...
                    match app_entry {
                        EntryTypes::HashChange(hash_change) => {
                            let result = validate_create_hash_change(
                                EntryCreationAction::Update(action.clone()),
                                hash_change.clone(),
                            )?;
                            if let ValidateCallbackResult::Valid = result {
                                let original_hash_change: Option<HashChange> = original_record
                                    .entry()
                                    .to_app_option()
                                    .map_err(|e| wasm_error!(e))?;
                                let original_hash_change = match original_hash_change {
                                    Some(hash_change) => hash_change,
                                    None => {
                                        return Ok(
                                            ValidateCallbackResult::Invalid(
