...
export const GET_FACET_GROUPS = gql`
...
export const GET_ALL_RESOURCE_SPECIFICATIONS = gql`
...
export const GET_ALL_PROCESS_SPECIFICATIONS = gql`
...
export const GET_ALL_FULL_PLANS = gql`
${PLAN_RETURN_FIELDS}
query {
  plans(last: 100000) {
    edges {
...
export const GET_NON_PROCESS_COMMITMENTS = gql`
...
export const GET_SIMPLIFIED_PLAN = gql`
${SIMPLIFIED_PLAN_RETURN_FIELDS}
query GetPlan($id: ID!) {
  plan(id: $id) {
    ...SimplifiedPlanReturnFields
...
export const GET_ALL_ECONOMIC_RESOURCES = gql`
...
export const GET_ALL_RECIPE_EXCHANGES = gql`
...
export const readFacetOfValue = async (facetValueId: string) => {
  console.log("reading facet of value", JSON.stringify(facetValueId))
  const facetValues = await client0.callZome({
    cap_secret: null,
    role_name: 'hrea_facets_0',
    zome_name: 'hc_facets',
    fn_name: 'retrieve_facet_values',
    payload: {identifier: facetValueId},
  })

  let facetValuesFormatted = facetValues.map((fv: any) => {
    return {
      ...fv,
      id: encodeHashToBase64(fv.id),
      revisionId: encodeHashToBase64(fv.revisionId),
      facetId: encodeHashToBase64(fv.facetId)
    }
  })
  console.log("facet values formatted", facetValuesFormatted)
  return facetValuesFormatted
}
...
export const getAllEconomicResources = async () => {
  ...
  const res = await client.query({
    query: GET_ALL_ECONOMIC_RESOURCES,
    fetchPolicy: 'no-cache'
  })
